...
class RangeSlider extends HTMLElement {
    ...
    get value() {
        return [this.min.valueAsNumber, this.max.valueAsNumber].sort(
            (a, b) => a - b
        );
    }
    ...
    addMarkup() {
        if (!this.querySelector('[type="range"]')) {
            let scope = "desktop";
            if (window.innerWidth < 990) scope = "mobile";
            ...
            this.insertAdjacentHTML(
                ...
                [
                    ...
                    '<input class="range" type="range"/>',
                    `<input class="filler" disabled type="range" id="filler_${scope}" name="filler_${scope}" />`,
                    '<label class="input-filler"></label>',
                    '<label class="input-range"></label>',
                    '<label class="input-range"></label>',
                ].join("")
            );
        }
    ...
    initLabelsElements() {
        ...
        labels.forEach((label, i) => {
            if (i == 0) {
                label.setAttribute("for", "filler" + this.scopeMaxElements);
                label.innerHTML = "Filler" + this.scopeMaxElements;
            } else if (i == 1) {
                label.setAttribute(
                    "for",
                    this.max.name + this.scopeMaxElements
                );
                label.innerHTML = this.max.name;
            } else {
                label.setAttribute(
                    "for",
                    this.min.name + this.scopeMinElements
                );
                label.innerHTML = this.min.name;
            }
        });
    }
    ...
    initMinMaxElements() {
        ...
        this.scopeMinElements = this.min.parentElement.dataset?.scope;
        this.scopeMaxElements = this.max.parentElement.dataset?.scope;
        ...
        if (name) {
            ...
            this.min.setAttribute("id", name + "[min]" + this.scopeMinElements);
            this.max.setAttribute("id", name + "[max]" + this.scopeMaxElements);
